...
var FN_RAISE = [
    'function __raise(file, line, block, msg, cause) {',
    ' if(cause) { msg += " (" + cause.message + ")"}',
    ' msg += "\\n    at " + __files[file] + ":" + line + ":" + block;',
    ' throw new Error(msg);',
    '}'
].join('');
...
/**
 * Representação de uma expressão executavel
 *
 * @param content
 * @param line
 * @param block
 * @constructor
 */
function Expression(content, line, block) {
 ...
 * @param text
 */
function parseExpressions(text, line, block, file) {
    text = text
        .replace(/(^[\s\n\r]+)/g, function ($0, $1) {
            var parts = $1.split('\n');
            line += parts.length - 1;
            block = parts.pop().length;
            return ''
        })
        .replace(/^\s*|\s*$/g, '');
                ...
                if (innerBrackets > 0) {
                    ...
                    expressions.push(
                        new Expression(
                            ...
                            '; } catch(e){ __raise(' + file + ',' + contentStartLine + ',' + contentStartBlock +
...
function parseTree(filePath, fileLoader, locals) {
    ...
    var line, file, block = 0;

    // Indica a posição (linha e bloco) do início do conteúdo
    var contentStartLine = 1;
    var contentStartBlock = 1;
            ...
            if (next === '/') {
                ...
                if (closeTag === ('/' + NODE.tag)) {
                    ...
                    if (contentTrim !== '') {
                        NODE.children.push({
                            tag: 'TEXT',
                            content: contentTrim,
                            expressions: parseExpressions(content, contentStartLine, contentStartBlock, file),
                            attributes: [],
                            children: [],
                            parent: NODE,
                            line: contentStartLine,
                            file: file,
                            block: contentStartBlock
                        });
                    }
                    contentClear();
                    NODE = NODE.parent;
                ...
                if (contentTrim !== '') {
                    NODE.children.push({
                        tag: 'TEXT',
                        content: contentTrim,
                        expressions: parseExpressions(content, line, block, file),
                        attributes: [],
                        children: [],
                        parent: NODE,
                        line: contentStartLine,
            ...
            if (inTagName) {
                ...
                if (c === '=' || (c === ' ' && contentTrim !== '')) {
                    ATTRIBUTE = {
                        ...
                        line: line,
                        file: file,
                        block: block - contentTrim.length
                    };
                    NODE.attributes.push(ATTRIBUTE);
                ...
                if (c === '"') {
                    ...
                    ATTRIBUTE.expressions = parseExpressions(content, line, block, file);
                    contentClear();
                    continue;
                }
            }
...
function compileFunctionBody(NODE, content, MIXINS) {
    switch (NODE.tag) {
        ...
        case 'if':
            ...
            }) || {value: 'false', line: 0, block: 0};

            var cond = getVariable();

            content += 'var ' + cond + ' = false; try { ' + cond + ' = ' + conditionAttr.value + '; } catch(e) {';
            content += ' __raise(' + conditionAttr.file + ',' + conditionAttr.line + ',' + conditionAttr.block +
