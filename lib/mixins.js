...
Mixins.prototype._processMixinCall = function (elementInfo, variables) {
    ...
    var mixinScopeVariables = _.clone(variables, true);

    // validação dos atributos de execução do mixin
    if (elementInfo.attributes) {
        var mixinArguments = {};
        elementInfo.attributes.replace(/([a-z_][a-z-_]*)="([^"]*)?"/gi, function ($0, argName, argValue) {
            argName = argName.replace(/-([a-z])/g, function ($0, $1) {
                return $1.toUpperCase();
            });

            if (mixin.params.indexOf(argName) >= 0) {
                // é uma variável definida
                if (argValue.match(/^(#{)[\s\S]*(})$/g)) {
                    argValue = mixinScopeVariables[argValue.replace(/^(#{)|(})$/g, '')];
                }

                mixinArguments[argName] = argValue;
                mixinScopeVariables[argName] = argValue;
            }
        });

        // verifica se todos os parametros obrigatórios do mixin foram setados
        _.each(mixin.params, function (param) {
            if (!mixinArguments.hasOwnProperty(param)) {
                throw new Error([
    ...
    elementInfo.children.forEach(function (child) {
        var childContent = that._processMixinCall(child, mixinScopeVariables);
        elementInfo.content = elementInfo.content.replace(child.text, childContent);
    });
    // adiciona a variavel conteudo ao mixin
    mixinScopeVariables.$content = elementInfo.content;

    // processa o template e os mixins filhos
    var mixinHtmlTemplate = mixin.template;
    mixinHtmlTemplate = varsUtil.processHtml(mixinHtmlTemplate, mixinScopeVariables);
    return this.processHtml(mixinHtmlTemplate, mixinScopeVariables);
};
